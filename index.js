...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:【Gustavo】\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558994263981:+55 899426-3981\n' //Nomor whatsapp kamu
            + 'END:VCARD'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[☢️] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD 【Gustavo】 BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de 【Gustavo】. Registre-se para fazer amizade com o bot 【Gustavo】 por meio, \n\nCommand : ${prefixbotbot}daftar nama|idade\nExemplo : ${prefixbotbot}daftar 【Gustavo】|17\n\n──「 【Gustavo】 BOT 」──`,
			...
			const ownerNumber = ["558994263981@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558994263981@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558994263981@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558994263981@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558994263981@s.whatsapp.net","559784130980@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net","12267740582@s.whatsapp.net"]
		...
		case 'tahta':
				...
				case 'water':
                 ...
                 case 'nomegp':
		             ...
		             break
				case 'loli2':
					if (!isAnime) return reply('⚠️ *Deve atitag o modo Anime* ⚠️')
    ...
    break
		...
		case 'bpfont':
			...
			break 
				...
				tag nin = gh.split("&")[0];
				tag ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「☢️」Contoh : ${prefixbotbot}ninjalogo Ramlan & Gans`)
				reply(ind.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				tag porn = gh.split("&")[0];
				tag hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「☢️」Contoh : ${prefixbotbot}pornhub Ramlan & Hub`)
				reply(mess.wait)
                ...
                case 'bloodtxt':
					...
					tag gem = gh.split("&")[0];
					tag bok = gh.split("&")[1];
					if (args.length < 1) return reply(`「☢️」Contoh : ${prefixbotbot}gemboktext NAKANO&MIKU`)
					reply(mess.wait)
                ...
                case 'glitchtext':
					...
					tag gli = gh.split("&")[0];
					tag tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「☢️」Contoh : ${prefixbotbot}glitchtext NAKANO&MIKU`)
					reply(ind.wait())
...
break
			    ...
			    case 'nsfwblowjob':
				    try {
						if (!isNsfw) return reply('⚠️ *FALSO* ⚠️')
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					dark.sendMess(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT 【Gustavo】 ✨* 」\n*│+ wa.me/558994263981*╰──────*「 *【Gustavo】* 」*────*\n\n*_SE QUER SER ADMIN DO BOT 【Gustavo】_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					dark.sendMess(from, sa, image, { quoted: mek, caption: '*╭────*「 *USERS CORNOS🐂* 」\n*│+ wa.me/558994263981*╰──────*「 * 【Gustavo】* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO 【Gustavo】 BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'dono':
					...
					dark.sendMess(from, bufferer, image, {quoted: mek, caption: '*CRIADOR:* 【Gustavo】⃖ 🤏😎\*WPP:* wa.me/+5589994263981\n*INSTA:* @gustavo2krp\n\n\nEspero que tenham gostado do bot 🤏😎️'})
					break
        ...
        case 'promover':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('⚠️ Falha ao converter adesivos em imagens ⚠️')
						bufferer = fs.readFileSync(ran)
						dark.sendMess(from, bufferer, image, {quoted: mek, caption: '>//<'})
						fs.unlinkSync(ran)
					})
